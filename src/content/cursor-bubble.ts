...
.domino-bubble.visible {
  opacity: 1;
  transform: scale(1) translateY(0);
}

.domino-bubble.fade-out {
  opacity: 0;
  transform: scale(0.96) translateY(4px);
}

/* Expanded width for answering state */
.domino-bubble.state-answering {
  width: 444px;
  max-height: 500px;
  padding: 20px 24px;
  border-radius: 18px;
...
.domino-bubble.state-transcribing,
.domino-bubble.state-planning {
  width: 320px;
  padding: 14px 16px;
  border-radius: 16px;
}
...
.domino-step-log::-webkit-scrollbar { width: 3px; }
.domino-step-log::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.12); border-radius: 2px; }
...
.domino-step-icon.done {
  background: rgba(255, 255, 255, 0.08);
  color: rgba(255, 255, 255, 0.5);
}

.domino-step-icon.active {
  background: rgba(255, 255, 255, 0.1);
  color: rgba(255, 255, 255, 0.6);
  animation: step-pulse 1.2s ease-in-out infinite;
}

.domino-step-icon.failed {
  background: rgba(255, 255, 255, 0.06);
  color: rgba(255, 255, 255, 0.35);
}

.domino-step-icon.thinking {
  background: rgba(255, 255, 255, 0.08);
...
.domino-step-text.result {
  color: rgba(255, 255, 255, 0.75);
}

.domino-step-text.failed {
  color: rgba(255, 255, 255, 0.45);
  font-style: italic;
}

.domino-step-text.thinking {
  color: rgba(255, 255, 255, 0.4);
  font-style: italic;
}

/* Fixed width for all non-listening states */
.domino-bubble.state-executing,
.domino-bubble.state-understanding,
.domino-bubble.state-error,
.domino-bubble.state-done {
  width: 320px;
  max-height: 420px;
  padding: 14px 16px;
  border-radius: 16px;
...
.domino-speaking-wave .wave-bar:nth-child(2)  { height: 14px; animation-delay: 0.08s; }
.domino-speaking-wave .wave-bar:nth-child(3)  { height: 20px; animation-delay: 0.12s; }
.domino-speaking-wave .wave-bar:nth-child(4)  { height: 26px; animation-delay: 0.16s; }
.domino-speaking-wave .wave-bar:nth-child(5)  { height: 32px; animation-delay: 0.2s; }
.domino-speaking-wave .wave-bar:nth-child(6)  { height: 28px; animation-delay: 0.24s; }
.domino-speaking-wave .wave-bar:nth-child(7)  { height: 34px; animation-delay: 0.28s; }
.domino-speaking-wave .wave-bar:nth-child(8)  { height: 24px; animation-delay: 0.32s; }
.domino-speaking-wave .wave-bar:nth-child(9)  { height: 30px; animation-delay: 0.36s; }
.domino-speaking-wave .wave-bar:nth-child(10) { height: 20px; animation-delay: 0.4s; }
.domino-speaking-wave .wave-bar:nth-child(11) { height: 26px; animation-delay: 0.44s; }
.domino-speaking-wave .wave-bar:nth-child(12) { height: 16px; animation-delay: 0.48s; }
.domino-speaking-wave .wave-bar:nth-child(13) { height: 10px; animation-delay: 0.52s; }
...
.domino-drag-handle:hover {
  background: rgba(255, 255, 255, 0.4);
}

.domino-drag-handle:active {
  cursor: grabbing;
  background: rgba(255, 255, 255, 0.5);
}

/* ─── Scrollbar styling ─── */
.domino-bubble::-webkit-scrollbar {
  width: 4px;
}

.domino-bubble::-webkit-scrollbar-track {
  background: transparent;
}

.domino-bubble::-webkit-scrollbar-thumb {
  background: rgba(255, 255, 255, 0.12);
  border-radius: 2px;
}

.domino-bubble::-webkit-scrollbar-thumb:hover {
  background: rgba(255, 255, 255, 0.2);
}
...
export class CursorBubble {
  ...
  show(cursorX: number, cursorY: number): void {
    ...
    this.container.id = 'domino-bubble-host';
    ...
    this.bubbleEl.className = 'domino-bubble';
    ...
    this.escapeHandler = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        e.preventDefault();
        e.stopPropagation();
        stopTts(); // Escape explicitly cuts off any spoken answer
        this.dismiss();
      }
    };
  ...
  setState(state: BubbleState, label?: string): void {
    if (state === 'idle') {
      // Don't tear down (which stops TTS) while the answer is still being spoken —
      // wait for playback to finish first, so the voice isn't cut off.
      if (isSpeaking()) {
        if (this.ttsPollTimer) clearInterval(this.ttsPollTimer);
        this.ttsPollTimer = setInterval(() => {
          if (!isSpeaking()) {
            if (this.ttsPollTimer) { clearInterval(this.ttsPollTimer); this.ttsPollTimer = null; }
            this.dismiss();
          }
        }, 300);
        return;
    ...
    if (state === 'answering') {
      ...
      if (stepLogEl) {
        const lastThinking = stepLogEl.querySelector('.domino-step-icon.thinking');
  ...
  setStep(name: string, index: number, total: number): void {
    ...
    if (isResult) {
      ...
      const stepLogEl = this.chatHistory.getStepLogEl();
      if (stepLogEl) {
        const lastActive = stepLogEl.querySelector('.domino-step-icon.active');
        if (lastActive) {
  ...
  speakSummary(summary: string): void {
    console.log('[DOMino][bubble] speakSummary called, displayMode:', this.displayMode, 'summary:', summary);
  ...
  private renderStatus(text: string): void {
    ...
    statusEl.className = 'domino-status';

    const dotEl = document.createElement('div');
    dotEl.className = 'domino-status-dot';
  ...
  private renderDone(): void {
    ...
    doneEl.className = 'domino-done';

    const checkEl = document.createElement('span');
    checkEl.className = 'domino-done-check';
    checkEl.textContent = '\u2713';
  ...
  private renderAnsweringState(): void {
    ...
    this.dragHandle.className = 'domino-drag-handle';
  ...
  private cleanup(): void {
    // NOTE: do NOT stop TTS here. The bubble auto-dismisses a couple seconds after
    // an answer, but the spoken audio is independent of the bubble DOM and should
    // play to completion. TTS is stopped explicitly on a new recording
    // (onHold/prepareForFollowUp) and on Escape — not on passive teardown.
    this.stopTracking();
    this.stopDrag();
