...
function sendMessage(msg: Record<string, unknown>): void {
  chrome.runtime.sendMessage(msg).catch((err) => console.error('[DOMino] message send:', err));
}
...
document.addEventListener('keydown', (e: KeyboardEvent) => {
  if (e.key === 'Escape' && bubble.isVisible()) {
    chrome.runtime.sendMessage({ action: 'cancel-agent-loop' }).catch((err) => console.error('[DOMino] cancel-agent-loop send (Escape):', err));
  }
}, true);
...
async function onHold(event: Event): Promise<void> {
  ...
  if (bubble.isVisible()) {
    chrome.runtime.sendMessage({ action: 'cancel-agent-loop' }).catch((err) => console.error('[DOMino] cancel-agent-loop send (hold):', err));
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.action === 'bubble-state') {
    ...
    if (!contentAmpLogged) {
      ...
      console.log('[DOMino][content] amplitude-data received, len=', message.data.length, 'max=', max, 'bubbleVisible=', bubble.isVisible());
    ...
    // where the domino-hold custom event fires on the wrong document)
    if (!bubble.isVisible()) {
      bubble.show(lastCursorX, lastCursorY);
      bubble.setState('listening');
    }
...
document.addEventListener('domino-hold', onHold);
document.addEventListener('domino-release', onRelease);

console.log('[DOMino] Content script loaded');
