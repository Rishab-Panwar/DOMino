...
export class WaveformRenderer {
  ...
  createWaveform(): HTMLDivElement {
    ...
    waveformEl.className = 'domino-waveform';
  ...
  updateAmplitude(frequencyData: Uint8Array): boolean {
    if (this.bars.length === 0) {
      if (this.ampLogCount < 3) {
        console.log('[DOMino][waveform] updateAmplitude SKIPPED: bars=', this.bars.length);
        this.ampLogCount++;
      }
      return false;
    }
    ...
    if (this.ampLogCount < 5) {
      ...
      console.log(`[DOMino][waveform] updateAmplitude applying: bins=${binCount} step=${step} max=${max} bars=${this.bars.length}`);
      this.ampLogCount++;
    }
