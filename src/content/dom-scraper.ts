export interface DomSnapshot {
  ...
  // Unified, indexed list of every actionable element on the page (set-of-marks).
  // The agent refers to elements by their numeric `index`, and the executor acts
  // on the exact registered element — no selector guessing. This is the primary
  // targeting mechanism; the arrays above are kept for context/back-compat.
...
export interface InteractiveElement {
  index: number;            // stable handle for this scrape
  tag: string;              // lowercase tag name
  kind: string;             // semantic kind: button | link | input | textarea | select | checkbox | radio | editable | option | other
  role?: string;            // classified purpose (add-to-cart, search-submit, etc.)
  label: string;            // best human label (text/aria/placeholder/value/title)
  value?: string;           // current value for form fields
  placeholder?: string;
  type?: string;            // input type
  href?: string;
  editable: boolean;        // contenteditable / textbox
  inViewport: boolean;
...
/* ------------------------------------------------------------------ */
/*  Interactive element registry (set-of-marks targeting)              */
/* ------------------------------------------------------------------ */

// Maps the index handed to the model -> the actual DOM element. Rebuilt on
// every scrape so the agent always acts on what it most recently observed.
const elementRegistry = new Map<number, Element>();

/** Resolve an element by the index assigned in the last scrape. */
export function getElementByIndex(index: number): Element | null {
  const el = elementRegistry.get(index);
  if (!el) return null;
  // Guard against elements detached since the scrape (DOM changed).
...
/** Classify an element into a coarse interaction kind. */
function elementKind(el: Element): string {
  const tag = el.tagName.toLowerCase();
  const role = el.getAttribute('role');
  if ((el as HTMLElement).isContentEditable || role === 'textbox') return 'editable';
...
/** Best human-readable label for an interactive element. */
function elementLabel(el: Element): string {
  ...
  if (placeholder) return trimText(placeholder, 120);
  if (el instanceof HTMLElement) {
    const label = findLabel(el);
    if (label) return label;
  }
  const value = (el as HTMLInputElement).value;
  if (value) return trimText(value, 120);
...
function scrapeInteractiveElements(): InteractiveElement[] {
  elementRegistry.clear();
  // Collect all visible interactive elements, then prioritize ones in the viewport.
  // Pages like Gmail have hundreds of interactive elements; without prioritization
  // a late-rendered, on-screen dialog (e.g. the compose window) would be cut by the
  // cap before it's reached. Stable sort keeps document order within each group.
  const all = Array.from(document.querySelectorAll(INTERACTIVE_SELECTOR)).filter(isVisible);
  all.sort((a, b) => (isInViewport(b) ? 1 : 0) - (isInViewport(a) ? 1 : 0));
  ...
  for (const el of all) {
    ...
    // Skip elements with no label AND no value/placeholder — usually noise.
    const inputEl = el as HTMLInputElement;
    const hasMeaning = label || inputEl.value || inputEl.placeholder || kind === 'input' || kind === 'editable' || kind === 'checkbox' || kind === 'radio';
    if (!hasMeaning) continue;

    elementRegistry.set(index, el);
    results.push({
      index,
      tag: el.tagName.toLowerCase(),
      kind,
      role: classifyElement(el, label) || undefined,
      label,
      value: inputEl.value || undefined,
      placeholder: inputEl.placeholder || undefined,
      type: el.tagName.toLowerCase() === 'input' ? (inputEl.type || 'text') : undefined,
      href: el.tagName.toLowerCase() === 'a' ? (el as HTMLAnchorElement).href || undefined : undefined,
      editable: (el as HTMLElement).isContentEditable || el.getAttribute('role') === 'textbox',
...
export function scrapeDom(): DomSnapshot {
  return {
    ...
    interactiveElements: safe(scrapeInteractiveElements, []),
...
function safe<T>(fn: () => T, fallback: T): T {
  try {
    ...
    console.warn(`[DOMino][dom-scraper] ${fn.name} failed:`, e);
    return fallback;
  }
}
