...
export class ChatHistoryManager {
  ...
  ensureTaskBanner(bubbleEl: HTMLDivElement): void {
    ...
    this.taskBannerEl.className = 'domino-task-banner';
  ...
  renderExecuting(bubbleEl: HTMLDivElement): HTMLDivElement {
    ...
    if (!this.stepLogEl || !bubbleEl.contains(this.stepLogEl)) {
      ...
      this.stepLogEl.className = 'domino-step-log';
  ...
  completeLastStep(): void {
    ...
    const lastActive = this.stepLogEl.querySelector('.domino-step-icon.active');
    if (lastActive) {
  ...
  showDoneSummary(bubbleEl: HTMLDivElement, steps?: string[]): void {
    ...
    const existing = bubbleEl.querySelector('.domino-done-summary');
    if (existing) existing.remove();

    if (stepsToShow.length === 0) return;

    const summaryEl = document.createElement('div');
    summaryEl.className = 'domino-done-summary';

    for (const step of stepsToShow) {
      const itemEl = document.createElement('div');
      itemEl.className = 'domino-done-summary-item';

      const checkEl = document.createElement('span');
      checkEl.className = 'domino-done-summary-check';
      checkEl.textContent = '\u2713';
  ...
  private renderStepEntryToLog(text: string, type: StepStatus): void {
    ...
    entry.className = 'domino-step-entry';

    const icon = document.createElement('div');
    icon.className = `domino-step-icon ${type}`;
    ...
    const textEl = document.createElement('span');
    textEl.className = `domino-step-text ${type === 'done' ? 'result' : type === 'failed' ? 'failed' : type === 'thinking' ? 'thinking' : ''}`;
