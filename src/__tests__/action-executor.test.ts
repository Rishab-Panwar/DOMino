...
describe('Action Executor', () => {
  ...
  describe('click action', () => {
    ...
    it('returns error when selector is missing', async () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.error).toContain('No element index or selector');
    });
    ...
    it('falls back to MouseEvent dispatch when .click() throws', async () => {
      ...
      expect(result.ok).toBe(true);
      // Falls back to a full mouse sequence (mousedown, mouseup, click)
      const types = mockEl.dispatchEvent.mock.calls.map((c: any[]) => c[0].type);
      expect(types).toContain('click');
      const clickEvent = mockEl.dispatchEvent.mock.calls.find((c: any[]) => c[0].type === 'click')[0];
      expect(clickEvent).toBeInstanceOf(MockMouseEvent);
