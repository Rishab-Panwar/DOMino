...
const KeyboardIllustration: React.FC = () => (
  <svg viewBox="0 0 200 140" className="w-48 h-auto mx-auto" aria-hidden="true">
    ...
    <circle cx="100" cy="74" r="60" fill="url(#keyGlow)" />
    <rect x="20" y="34" width="160" height="92" rx="12" fill="url(#cardGrad)" stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
    {/* dim keys: full top row, left side of the next two rows */}
    {[0, 1, 2].map((row) => {
      const cols = row === 0 ? 8 : 6;
      return Array.from({ length: cols }).map((_, col) => (
        <rect key={`${row}-${col}`} x={30 + col * 18} y={44 + row * 18} width={14} height={14} rx={3}
          fill="rgba(255,255,255,0.08)" opacity={0.6} />
      ));
    })}
    {/* highlighted Enter key (right side, spans the lower two rows) */}
    <rect x="134" y="62" width="34" height="32" rx={4} fill="#FFFFFF">
      <animate attributeName="opacity" values="1;0.6;1" dur="1.5s" repeatCount="indefinite" />
    </rect>
    <text x="151" y="84" textAnchor="middle" fill="#111" fontSize="18" fontFamily="monospace" fontWeight="bold">{'↵'}</text>
    {/* floating cursor + click ripple over the Enter key */}
    <g>
      ...
      <circle cx="151" cy="70" r="6" fill="#E0E0E0" opacity="0.4" />
      <circle cx="151" cy="70" r="3" fill="#FFFFFF" opacity="0.7" />
    </g>
    <circle cx="151" cy="86" r="8" fill="none" stroke="#FFFFFF" strokeWidth="1" opacity="0">
  ...
  return (
    <div className="welcome-root">
      ...
      <div className="welcome-container">
        <h2 className="welcome-title">DOM<span className="gradient-text">ino</span></h2>
        <p className="welcome-subtitle">
          <b>DOM</b>, the structure of every page it operates on. <b>Domino</b>, one command topples a chain of actions that fall into place.
        </p>
          ...
          <div key={step} className="card-content step-enter">
            {step === 1 && (
              <>
                ...
                <p className="card-desc">
                  Press and hold <kbd className="key-badge">{shortcutKey === ' ' ? 'Space' : shortcutKey}</kbd> and speak a command, DOMino sees your screen and does it: click, type, navigate, and complete tasks. Release when you're done.
                </p>
            ...
            {step === 2 && (
              <>
                ...
                <p className="card-desc">DOMino only listens while you hold the key. Your audio is never stored or sent anywhere except to process your question.</p>
                {micError && (
        ...
        <p className="footer-text">DOMino</p>
      </div>
      ...
      <style>{`
        ...
        .welcome-subtitle b { color: rgba(255, 255, 255, 0.85); }
