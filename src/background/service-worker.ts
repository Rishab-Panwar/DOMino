  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0]?.id) {
      chrome.tabs.sendMessage(tabs[0].id, {
        ...
        state,
      }).catch((err) => console.error('[DOMino] state-changed broadcast:', err));
    }
  });
}
...
async function runPipeline(tabId: number, audioBase64: string, mimeType: string): Promise<void> {
  ...
  try {
    ...
    if (domResponse?.ok && (domResponse as { ok: boolean; snapshot?: DomSnapshot }).snapshot) {
      ...
      console.warn('[DOMino] Could not scrape DOM, proceeding without');
    }
    ...
    if (taskResult.type === 'answer') {
      ...
      // Speak the full answer (capped to keep TTS latency/cost reasonable)
      const summary = answerText.length > 1500 ? answerText.slice(0, 1500) : answerText;
      ...
      console.log('[DOMino] Executing action plan:', taskResult.actions);
    ...
    console.error('[DOMino] Pipeline error:', err);
...
async function runFollowUp(tabId: number, text: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.warn('[DOMino] Could not scrape DOM for follow-up, proceeding without');
    }
    ...
    if (taskResult.type === 'answer') {
      ...
      console.log('[DOMino] Executing follow-up action plan:', taskResult.actions);
    ...
    console.error('[DOMino] Follow-up error:', err);
    sendToTab(tabId, { action: 'pipeline-error', error: 'Something went wrong — give it another try' });
  } finally {
    pipelineRunning = false;
    currentState = 'idle';
...
registerHandler('shortcut-hold', (message, sender, sendResponse) => {
  console.log('[DOMino][SW] Received shortcut-hold, conversationState:', conversation.getState());
  ...
  recordingStartedPromise = ensureOffscreen().then(() => {
    console.log('[DOMino][SW] Sending start-recording to offscreen');
    return chrome.runtime.sendMessage({ target: 'offscreen', action: 'start-recording' }).catch((err) => console.error('[DOMino] start-recording send:', err));
  }).then(() => {
    console.log('[DOMino][SW] start-recording message sent successfully');
  }).catch((err) => {
    console.error('[DOMino][SW] Failed to create offscreen document:', err);
  });
  sendResponse({ ok: true, state: currentState });
});

registerHandler('shortcut-release', (message, sender, sendResponse) => {
  console.log('[DOMino][SW] Received shortcut-release');
  currentState = 'processing';
  broadcastStateChange(currentState);
  ...
  startPromise.then(() => {
    console.log('[DOMino][SW] Sending stop-recording to offscreen');
    chrome.runtime.sendMessage({ target: 'offscreen', action: 'stop-recording' }).catch((err) => {
      console.error('[DOMino][SW] Failed to send stop-recording:', err);
    });
  });
...
registerHandler('offscreen-started', () => {
  console.log('[DOMino][SW] Offscreen recording started');
});
...
registerHandler('offscreen-recording-complete', (message) => {
  ...
  console.log('[DOMino][SW] Offscreen recording complete, audioBase64 length:', msg.audioBase64?.length, 'mimeType:', msg.mimeType, 'pendingTabId:', pendingTabId);
  const tabId = pendingTabId;
  pendingTabId = null;
  if (tabId && msg.audioBase64) {
    console.log('[DOMino][SW] Calling runPipeline with tabId:', tabId);
    runPipeline(tabId, msg.audioBase64, msg.mimeType);
  } else {
    console.error('[DOMino][SW] Cannot run pipeline: tabId=', tabId, 'hasAudio=', !!msg.audioBase64);
  }
});

registerHandler('offscreen-error', (message) => {
  const errMsg = (message as unknown as { action: 'offscreen-error'; error: string }).error;
  console.error('[DOMino] Offscreen error:', errMsg);
  if (pendingTabId) {
  ...
  fetch(`https://api.elevenlabs.io/v1/text-to-speech/${ttsMsg.voiceId}/stream`, {
    ...
    .then(async (resp) => {
      if (!resp.ok) {
        ...
        console.warn(`[DOMino] ElevenLabs TTS API error: HTTP ${resp.status} voiceId=${ttsMsg.voiceId} ${body.slice(0, 200)}`);
        sendResponse({ ok: false, error: `HTTP ${resp.status}` });
        return;
      }
      console.log(`[DOMino] ElevenLabs TTS API OK ✅ voiceId=${ttsMsg.voiceId} model=${ttsMsg.modelId}`);
    ...
    .catch((err) => {
      console.warn('[DOMino] ElevenLabs TTS fetch threw:', err);
  ...
  if (conversation.getState() === ConversationState.AwaitingReply) {
    ...
    setTimeout(async () => {
      ...
      chrome.runtime.sendMessage({ target: 'offscreen', action: 'start-recording' })
        .catch(err => console.error('[DOMino] auto-reopen start-recording:', err));
      conversation.transition(ConversationState.Listening);
      ...
      setTimeout(() => {
        if (conversation.getState() === ConversationState.Listening) {
          chrome.runtime.sendMessage({ target: 'offscreen', action: 'stop-recording' })
            .catch(err => console.error('[DOMino] auto-stop recording:', err));
        }
      }, 10_000);
    }, 500);
  }
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === 'local' && changes['domino-mic-granted']) {
    resolveIconState();
  }
});
...
function initSSE(): void {
  ...
  sseConnection.addEventListener('status', (event: MessageEvent) => {
    try {
      ...
      console.log('[DOMino] SSE status:', data.stage, data);
      ...
      console.warn('[DOMino] Failed to parse SSE event:', event.data);
    }
  });
  sseConnection.onerror = () => {
    console.warn('[DOMino] SSE connection lost, will retry in 5s');
    sseConnection?.close();
    sseConnection = null;
...
ensureOffscreen().then(() => {
  console.log('[DOMino] Offscreen document pre-created (mic ready)');
}).catch(() => {
...
checkBackendHealth().then((ok) => {
  if (ok) {
    initSSE();
    console.log('[DOMino] Backend connected, SSE initialized');
  } else {
    console.warn('[DOMino] Backend not reachable at localhost:8000 — start it with: cd backend && uvicorn backend.main:app');
  }
});
