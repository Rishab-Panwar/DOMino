export async function captureScreenshot(tabId?: number): Promise<string> {
  ...
  try {
    ...
    if (tabId) {
      await chrome.tabs.sendMessage(tabId, { action: 'show-overlay' }).catch((err) => console.error('[DOMino] overlay restore after capture:', err));
    }
    ...
    if (tabId) {
      await chrome.tabs.sendMessage(tabId, { action: 'show-overlay' }).catch((err) => console.error('[DOMino] overlay restore after capture error:', err));
    }
    throw err;
  }
}
