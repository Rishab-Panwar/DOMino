...
export function handleOffscreenReady(): void {
  console.log('[DOMino][SW] Offscreen document script loaded');
...
export function forwardAmplitude(
  ...
  if (recordingTabId) {
    if (!swAmpLogged) {
      console.log('[DOMino][SW] forwarding amplitude to tab', recordingTabId, 'dataLen=', data?.length, 'first8=', data?.slice(0, 8));
    }
    chrome.tabs.sendMessage(recordingTabId, {
      action: 'amplitude-data',
      data,
    }).catch((err) => console.error('[DOMino] amplitude-data forward to tab:', err));
  } else {
    console.warn('[DOMino][SW] amplitude received but recordingTabId is null');
  }
...
async function _ensureOffscreen(): Promise<void> {
  ...
  console.log('[DOMino][SW] Creating offscreen document...');
  ...
  offscreenReady = true;
  console.log('[DOMino][SW] Offscreen document ready');
}
