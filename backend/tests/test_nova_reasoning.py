...
class TestContinuePromptSelectorRules:
    """Verify that CONTINUE_SYSTEM_PROMPT contains the index-based targeting rules."""

    def test_continue_prompt_contains_index_rules(self):
        """CONTINUE_SYSTEM_PROMPT must instruct targeting by element index."""
        prompt = CONTINUE_SYSTEM_PROMPT.lower()
        assert "index" in prompt and "interactiveelements" in prompt

    def test_continue_prompt_lists_universal_actions(self):
        """CONTINUE_SYSTEM_PROMPT should document the universal action vocabulary."""
        prompt = CONTINUE_SYSTEM_PROMPT.lower()
